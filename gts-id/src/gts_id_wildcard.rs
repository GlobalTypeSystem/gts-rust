...
//!
//! [`GtsIdWildcard`] is a validated GTS identifier that may end in a single `*`
//! token. It supports prefix-based [`overlaps`](GtsIdWildcard::overlaps) and
//! [`is_subset_of`](GtsIdWildcard::is_subset_of) reasoning; actual ID-vs-pattern
...
impl GtsIdWildcard {
    ...
    ///
    /// ```ignore
    /// let broad  = GtsIdWildcard::new("gts.x.core.srr.resource.v1~*")?;
    /// let narrow = GtsIdWildcard::new("gts.x.core.srr.resource.v1~acme.*")?;
    /// assert!(broad.overlaps(&narrow));  // "acme.*" is a subset of "*"
    ///
    /// let other  = GtsIdWildcard::new("gts.x.core.other.resource.v1~*")?;
    /// assert!(!broad.overlaps(&other));  // different base type — no overlap
    /// ```
    #[must_use]
    pub fn overlaps(&self, other: &GtsIdWildcard) -> bool {
    ...
    /// ```
    #[must_use]
    pub fn is_subset_of(&self, other: &GtsIdWildcard) -> bool {
        let a = Self::prefix_str(&self.id);
        let b = Self::prefix_str(&other.id);
        a.starts_with(b)
    }
    ...
    /// ([`crate::parse::parse_gts_id`] with wildcards enabled), so a wildcard
    ...
    pub fn new(pattern: &str) -> Result<Self, GtsIdError> {
        ...
        // contain a trailing '*'.
        let segments = crate::parse::parse_gts_id(p, true)?;
...
impl fmt::Display for GtsIdWildcard {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "{}", self.id)
    }
}

impl FromStr for GtsIdWildcard {
    type Err = GtsIdError;
...
impl AsRef<str> for GtsIdWildcard {
    fn as_ref(&self) -> &str {
        &self.id
    }
}
...
mod tests {
    ...
    fn test_gts_wildcard_simple() {
        let pattern = GtsIdWildcard::new("gts.x.core.events.*").expect("test");
    ...
    fn test_version_flexibility_in_matching() {
        ...
        let pattern = GtsIdWildcard::new("gts.x.core.events.event.v1~").expect("test");
    ...
    fn test_gts_wildcard_version_mismatch() {
        let pattern = GtsIdWildcard::new("gts.x.core.events.event.v2~").expect("test");
        let id = GtsId::new("gts.x.core.events.event.v1~").expect("test");
        assert!(!id.wildcard_match(&pattern));
    }
    ...
    fn test_gts_wildcard_with_minor_version() {
        let pattern = GtsIdWildcard::new("gts.x.core.events.event.v1.0~").expect("test");
    ...
    fn test_gts_wildcard_invalid_pattern() {
        let result = GtsIdWildcard::new("invalid");
        assert!(result.is_err());
    }

    #[test]
    fn test_gts_wildcard_multiple_wildcards_error() {
        let result = GtsIdWildcard::new("gts.*.*.*.*");
        assert!(result.is_err());
    }
    ...
    fn test_gts_wildcard_only_at_end() {
        ...
        let result1 = GtsIdWildcard::new("gts.*.core.events.event.v1~");
        assert!(result1.is_err());

        // Wildcard at end should work
        let pattern2 = GtsIdWildcard::new("gts.x.core.events.*").expect("test");
    ...
    fn test_gts_wildcard_from_str_trait() {
        let pattern: GtsIdWildcard = "gts.x.core.events.*".parse().expect("test");
        assert_eq!(pattern.id, "gts.x.core.events.*");
    }
    ...
    fn test_gts_wildcard_type_suffix_match() {
        ...
        let pattern = GtsIdWildcard::new("gts.x.pkg.ns.type.v1~*").expect("test");
    ...
    fn test_gts_wildcard_at_various_positions() {
        ...
        let result = GtsIdWildcard::new("gts.x.*");
        assert!(result.is_ok());

        // Wildcard at namespace position
        let result = GtsIdWildcard::new("gts.x.pkg.*");
        assert!(result.is_ok());

        // Wildcard at type position
        let result = GtsIdWildcard::new("gts.x.pkg.ns.*");
        assert!(result.is_ok());

        // Wildcard at version position
        let result = GtsIdWildcard::new("gts.x.pkg.ns.type.*");
        assert!(result.is_ok());
    }
    ...
    fn test_overlaps_broad_and_narrow() {
        let broad = GtsIdWildcard::new("gts.x.core.srr.resource.v1~*").expect("test");
        let narrow = GtsIdWildcard::new("gts.x.core.srr.resource.v1~acme.*").expect("test");
    ...
    fn test_overlaps_disjoint_types() {
        let a = GtsIdWildcard::new("gts.x.core.srr.resource.v1~*").expect("test");
        let b = GtsIdWildcard::new("gts.x.core.other.resource.v1~*").expect("test");
        assert!(!a.overlaps(&b));
        assert!(!b.overlaps(&a));
    }
    ...
    fn test_overlaps_same_pattern() {
        let a = GtsIdWildcard::new("gts.x.core.srr.resource.v1~*").expect("test");
        let b = GtsIdWildcard::new("gts.x.core.srr.resource.v1~*").expect("test");
        assert!(a.overlaps(&b));
    }
    ...
    fn test_overlaps_exact_vs_wildcard() {
        let exact =
            GtsIdWildcard::new("gts.x.core.srr.resource.v1~acme.crm._.contact.v1~").expect("test");
        let broad = GtsIdWildcard::new("gts.x.core.srr.resource.v1~*").expect("test");
    ...
    fn test_overlaps_tilde_star_chain() {
        ...
        let base = GtsIdWildcard::new("gts.x.core.srr.resource.v1~*").expect("test");
        let sub = GtsIdWildcard::new("gts.x.core.srr.resource.v1~acme.crm.*").expect("test");
    ...
    fn test_subset_three_levels() {
        let l1 = GtsIdWildcard::new("gts.x.core.srr.resource.v1~*").expect("test");
        let l2 = GtsIdWildcard::new("gts.x.core.srr.resource.v1~acme.*").expect("test");
        let l3 = GtsIdWildcard::new("gts.x.core.srr.resource.v1~acme.crm.*").expect("test");
